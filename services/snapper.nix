...
let
  ...
  createSnapshotsSubvolumeScripts = lib.forEach relevantConfigs (
    configInfo:
  ...
  snapperWrappers = lib.mapAttrsToList (
    name: _:
...
in
{
  ...
  environment.systemPackages = [ pkgs.snapper ] ++ snapperWrappers;

  systemd.services.snapperd.serviceConfig.ExecStartPre = lib.mkBefore (
    lib.optionals (createSnapshotsSubvolumeScripts != [ ]) createSnapshotsSubvolumeScripts
  );
}
