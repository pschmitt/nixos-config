#!/usr/bin/env bash

github_age_keys() {
  local user="$1"
  local url="https://github.com/${user:-${GITHUB_USERNAME}}.keys"

  curl -fsSL "$url" | grep ed25519 | xargs -I {} bash -c 'ssh-to-age <<< "{}"'
}

# shellcheck disable=2120
github_age_keys_yaml() {
  local gh_keys hosts
  mapfile -t gh_keys < <(github_age_keys "$@")

  local key
  for key in "${gh_keys[@]}"
  do
    echo "- ${key} # github/${GITHUB_USERNAME}"
  done
}

nix_host_configs() {
  # NOTE we could inspect the flake but there is not easy way to filter out
  # non-physical hosts such as "iso" or "pxe"
  find ./hosts -mindepth 2 -maxdepth 2 -iname "secrets.sops.yaml" \
    -exec sh -c 'i="$1"; basename "$(dirname "$i")"' shell {} \; | sort -u
}

nix_hm_enabled_hosts() {
  # shellcheck disable=2016
  nix eval --json '.#nixosConfigurations' --apply '
    cfgs: builtins.filter (n: cfgs.${n}.config.home-manager.enabled or false)
    (builtins.attrNames cfgs)
   ' | jq -r 'sort | .[]'
}

hm_host_age_keys_yaml() {
  local -a hm_hosts
  mapfile -t hm_hosts < <(nix_hm_enabled_hosts)

  local host host_age_key
  for host in "${hm_hosts[@]}"
  do
    host_age_key=$(host_age_key "$host")
    echo "- ${host_age_key} # host key of ${host}"
  done
}

initrd_host_age_keys_yaml() {
  local -a hosts
  mapfile -t hosts < <(nix_host_configs)

  local host initrd_age_key
  for host in "${hosts[@]}"
  do
    if initrd_age_key=$(initrd_host_age_key "$host" 2>/dev/null)
    then
      echo "- ${initrd_age_key} # initrd host key of ${host}"
    fi
  done
}

host_pubkey() {
  local host="$1"

  # sops
  sops --decrypt --extract '["ssh"]["host_keys"]["ed25519"]["pubkey"]' \
    "./hosts/${host}/secrets.sops.yaml"
}

host_privkey() {
  local host="$1"

  # sops
  sops --decrypt --extract '["ssh"]["host_keys"]["ed25519"]["privkey"]' \
    "./hosts/${host}/secrets.sops.yaml"
}

host_age_key() {
  host_pubkey "$1" | ssh-to-age
}

initrd_host_pubkey() {
  local host="$1"

  sops --decrypt --extract '["ssh"]["initrd_host_keys"]["ed25519"]["pubkey"]' \
    "./hosts/${host}/secrets.sops.yaml"
}

initrd_host_age_key() {
  initrd_host_pubkey "$1" | ssh-to-age
}

hermes_sops_age_key() {
  ssh-to-age < ./secrets/hermes-sops.pub
}

sops_config_gen() {
  local hosts
  mapfile -t hosts < <(nix_host_configs)

  local gh_keys
  gh_keys=$(github_age_keys_yaml)

  if [[ -z $gh_keys ]]
  then
    echo "No github keys found for user ${GITHUB_USERNAME}" >&2
    return 1
  fi

  local hm_keys
  hm_keys=$(hm_host_age_keys_yaml)

  local initrd_host_keys
  initrd_host_keys=$(initrd_host_age_keys_yaml)
  if [[ -z "$initrd_host_keys" ]]
  then
    initrd_host_keys='[]'
  fi

  local hermes_age_key
  hermes_age_key=$(hermes_sops_age_key)
  if [[ -z "$hermes_age_key" ]]
  then
    echo "No Hermes SOPS age key found" >&2
    return 1
  fi

  local sops_config
  sops_config=$(gh_keys="$gh_keys" hm_keys="$hm_keys" initrd_host_keys="$initrd_host_keys" hermes_age_key="$hermes_age_key" \
    yq -n '
    {
      "creation_rules": [
        {
          "path_regex": "secrets/shared.*",
          "key_groups": [{"age": env(gh_keys) + [strenv(hermes_age_key)]}]
        },
        {
          "path_regex": "secrets/initrd-wifi.sops.yaml",
          "key_groups": [{"age": env(gh_keys) + env(initrd_host_keys)}]
        },
        {
          "path_regex": "secrets/gpg.sops.yaml",
          "key_groups": [{"age": env(gh_keys) + env(hm_keys)}]
        }
      ]
    }
  ')

  local host creation_rules host_age_key
  for host in "${hosts[@]}"
  do
    host_age_key=$(host_age_key "$host")
    creation_rules=$(host="$host" host_age_key="$host_age_key" gh_keys="$gh_keys" \
    yq -ern '{
      "path_regex": "hosts/" + strenv(host) + "/secrets.sops.yaml",
      "key_groups": [
        {
          "age": [
            strenv(host_age_key)
          ] + env(gh_keys)
         }
       ]
     } | .key_groups[0].age[0] line_comment="host key of " + strenv(host)')

    if [[ "$host" == rofl-10 ]]
    then
      creation_rules=$(hermes_age_key="$hermes_age_key" r="$creation_rules" \
        yq -er <<< "$creation_rules" '
          .key_groups[0].age += [strenv(hermes_age_key)] |
          .key_groups[0].age[-1] line_comment="Hermes SOPS key"
        ')

      local luks_creation_rule
      luks_creation_rule=$(host="$host" host_age_key="$host_age_key" gh_keys="$gh_keys" hermes_age_key="$hermes_age_key" \
        yq -ern '
          {
            "path_regex": "hosts/" + strenv(host) + "/luks.sops.yaml",
            "key_groups": [
              {
                "age": [
                  strenv(host_age_key)
                ] + env(gh_keys) + [strenv(hermes_age_key)]
              }
            ]
          }
          | .key_groups[0].age[0] line_comment="host key of " + strenv(host)
          | .key_groups[0].age[-1] line_comment="Hermes SOPS key"
        ')
      sops_config=$(r="$luks_creation_rule" sops_config="$sops_config" \
        yq -er <<< "$sops_config" '.creation_rules += env(r)')
    fi

    # Add host-specific creation rules and append current host age key to the
    # list of shared secret recipients
    sops_config=$(r="$creation_rules" host="$host" host_age_key="$host_age_key" \
      yq -er <<< "$sops_config" '
        .creation_rules += env(r) |
        .creation_rules[0].key_groups[0].age += [strenv(host_age_key)] |
        .creation_rules[0].key_groups[0].age[-1] line_comment="host key of " + strenv(host)
      ')

  done

  # append default config
  gh_keys="$gh_keys" yq -er <<< "$sops_config" '
    .creation_rules += [{"key_groups": [{"age": env(gh_keys)}]}] |
    .creation_rules[-1] headComment="Default rules"'
  # echo "$sops_config"
}

main() {
  set -e -o pipefail
  # go to the root of the repo
  cd "$(cd "$(dirname "$0")/.." >/dev/null 2>&1; pwd -P)" || exit 9

  if ! command -v nix >/dev/null
  then
    echo "Error: nix is required to discover Home Manager hosts" >&2
    return 1
  fi

  GITHUB_USERNAME="${GITHUB_USERNAME:-$(whoami)}"

  while [[ -n "$*" ]]
  do
    case "$1" in
      -g|--username|--github-user*)
        if [[ -n "$2" ]]; then
          GITHUB_USERNAME="$2"
          shift 2
        else
          echo "Error: Missing value for $1" >&2
          exit 1
        fi
        ;;
      -u|--update*)
        SOPS_UPDATE_KEYS=1
        shift
        ;;
      -a|--auto)
        SOPS_UPDATE_KEYS=1
        SOPS_GEN_SSH_AUTH_KEYS=1
        shift
        ;;
      *)
        break
        ;;
    esac
  done

  SOPS_CONFIG_TMPFILE="$(mktemp)"
  SOPS_CONFIG_FILE="${1:-${PWD}/.sops.yaml}"

  local generated_config
  if ! generated_config=$(sops_config_gen)
  then
    echo "Error: failed to generate SOPS configuration; existing file was not changed" >&2
    rm -f "$SOPS_CONFIG_TMPFILE"
    return 1
  fi

  if ! yq -e '.creation_rules | type == "!!seq" and length > 0' \
    <<< "$generated_config" >/dev/null
  then
    echo "Error: generated SOPS configuration is invalid; existing file was not changed" >&2
    rm -f "$SOPS_CONFIG_TMPFILE"
    return 1
  fi

  {
    printf '# generated by %s on %s\n\n' "$(basename "$0")" "$(date -Iseconds)"
    printf '%s\n' "$generated_config"
    printf '\n# vim: set ft=yaml :\n'
  } > "$SOPS_CONFIG_TMPFILE"

  if ! mv "$SOPS_CONFIG_TMPFILE" "$SOPS_CONFIG_FILE"
  then
    echo "Failed to write sops config to $SOPS_CONFIG_FILE" >&2
    return 1
  fi

  echo "✅ sops config was written to $SOPS_CONFIG_FILE"
  [[ -z "$SOPS_UPDATE_KEYS" ]] && return 0

  ./secrets/sops-update-keys.sh
  local rc="$?"

  [[ -z "$SOPS_GEN_SSH_AUTH_KEYS" ]] && return "$rc"

  ./secrets/ssh-gen-known-hosts.sh
}

if [[ "${BASH_SOURCE[0]}" == "${0}" ]]
then
  main "$@"
fi

# vim: set ft=sh et ts=2 sw=2 :
