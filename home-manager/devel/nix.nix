{ config, pkgs, ... }:
{
  ...
  sops = {
    secrets = {
      "nix/github_token".sopsFile = config.host.sopsDefaultFile;
      # Also declared by work/glab.nix; repeated here since the token
      # template below needs it on hosts without the work profile (eg.
      # nix-on-droid).
      "glab/git.wiit.one/token".sopsFile = config.host.sopsDefaultFile;
    };
