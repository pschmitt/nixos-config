...
source /run/secrets/rendered/go-hass-agent.env || exit 9
...
then
  echo "HASS_SERVER or HASS_TOKEN not set from environment" >&2
  exit 2
fi

if ! command -v websocat >/dev/null
then
  echo "websocat not found in PATH" >&2
  exit 2
fi
...
if ! jq -e 'select(.id == 1 and .type == "result" and .success == true)' <<<"$devices_json" > /dev/null
then
