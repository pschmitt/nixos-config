...
let
  ...
  mv = combo: args: bindOpts combo (exec "zhj mouse::move ${args}") { repeating = true; };
  cl = combo: btn: execBind combo ("zhj mouse::click" + lib.optionalString (btn != null) " ${btn}");
  mouseto =
    combo: x: y:
...
in
{
  ...
  wayland.windowManager.hyprland = {
    settings.bind = [
      ...
      (execBind "SUPER + ALT + a" "~/bin/obs.zsh alt")
      (execBind "SUPER + ALT + w" "~/bin/obs.zsh webcam")
      (execBind "SUPER + ALT + f" "~/bin/obs.zsh freeze")
      (execBind "SUPER + ALT + up" "~/bin/obs.zsh thumbs-up")
      (execBind "SUPER + ALT + down" "~/bin/obs.zsh thumbs-down")
      (execBind "SUPER + ALT + period" "~/bin/obs.zsh emoji")
      (execBind "SUPER + R" "~/bin/wofi.zsh run")
      (execBind "SUPER + period" "~/bin/wofi.zsh emoji")
      ...
      (execBind "SUPER + ALT + s" "~/bin/wofi.zsh soundboard")
      (execBind "SUPER + SHIFT + s" "~/bin/wofi.zsh soundboard stop")
      ...
      (execBindLocked "XF86AudioMicMute" "~/bin/obs.zsh toggle-mute")
      (execBindLocked "SUPER + space" "~/bin/obs.zsh toggle-mute")
      ...
      (execBindLocked "SUPER + ALT + b" "~/bin/obs.zsh brb --mute")
    ]
    ++ wsBinds;
