...
let
  ...
  # JSON of all Nix-managed config sections — used both for hash-tracking and
  # as the argument to the Python updater so that changes to exclude, modules,
  # or bash.expose all trigger a config sync.
  ...
  nixConfigUpdaterPy = pkgs.writeText "lnxlink-update-nix-config.py" ''
    ...
    # Sync Nix-managed top-level fields; use None for empty lists so lnxlink
    # treats them as "auto" (same as the initial YAML null value).
  ...
  # Initial config written once to a writable path so lnxlink can persist
  # module-specific settings under the `settings:` key. MQTT credentials and
  # connection parameters are always sourced from environment variables (see
  # sops.templates."lnxlink.env") so the static file never holds secrets.
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    xdg.configFile = mkScriptFiles ./lnxlink/scripts;

    # Write the initial config to a writable path on first activation.
    # lnxlink may append module-specific settings under `settings:` at runtime.
    # All Nix-managed sections (exclude, modules, bash.expose, …) are kept in
    # sync on every switch via a hash-tracked Python updater.
    ...
    systemd.user.services.lnxlink = {
      ...
      Service = {
        ...
        # Strip URI scheme (e.g. tcp://) and port suffix from LNXLINK_MQTT_SERVER
        # so it works whether the secret holds a plain hostname or a full URI.
