resource "openstack_networking_addressscope_v2" "bgpv6_legacy" {
...
resource "openstack_networking_subnetpool_v2" "customer_ipv6_legacy" {
  provider         = openstack.optimist-legacy
  name             = "customer-ipv6"
  prefixes         = ["2a00:c320:1000::/48"]
  address_scope_id = openstack_networking_addressscope_v2.bgpv6_legacy.id
  is_default       = true
  shared           = true
}
